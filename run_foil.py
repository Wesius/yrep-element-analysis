#!/usr/bin/env python3
"""Run the composable analysis pipeline on the foil measurements (Nov 5)."""
...
# Configuration tuned for aluminum foil
VISUALIZE = True
PLOT_DIR = Path(__file__).parent / "plots/foil"
AVERAGE_POINTS = 1200
...
# Expected elements for household aluminum foil and common contaminants
FOIL_ELEMENTS = [
    "Al",
    "Mg",
    "Si",
...
def load_runs(root: Path) -> dict[str, list[Signal]]:
    """Load spectra grouped by immediate subdirectories (Foil_1, Foil_2...)."""
...
def main() -> None:
    ...
    species_filter = expand_species_filter(references.lines.keys(), FOIL_ELEMENTS)

    print("Loading foil runs...")
    runs = load_runs(foil_dir)
    if not runs:
        print("No foil measurements found.")
        return
