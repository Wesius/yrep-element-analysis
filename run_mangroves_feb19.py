#!/usr/bin/env python3
"""Run the composable analysis pipeline on the Feb 19 mangroves datasets."""
...
# Configuration
VISUALIZE = True
PLOT_DIR = Path(__file__).parent / "plots/mangroves_feb19"
AVERAGE_POINTS = 1200
...
def load_runs(root: Path) -> dict[str, list[Signal]]:
    """Load spectra grouped by immediate subdirectories."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Feb 19 mangroves analyses")
    parser.add_argument(
        "--category",
        default="all",
        choices=["all", "Soil", "Propagules"],
        help="Which category to run",
    )
    args = parser.parse_args()

    base = Path(__file__).resolve().parent
    data_root = base / "data" / "mangroves_feb19"
    lists_dir = base / "data" / "lists"

    categories = {
        "Soil": data_root / "Soil",
        "Propagules": data_root / "Propagules",
    }
    ...
    if VISUALIZE:
        print("\nPlots saved under plots/mangroves_feb19/<Category>/<Dataset>/<Run>_*.png")
